...
export default async function fetchRefresh(fetch: (input: URL | RequestInfo, init?: RequestInit | undefined) => Promise<Response>,
path: string)
{
    ...
    if(res.status == 401){
        ...
        if(!refreshRes.ok) throw console.error(401, 'Session expired');
            return fetch(path);
